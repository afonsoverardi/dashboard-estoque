...
# Memória para o zoom e para os filtros de checkbox
if 'item_para_zoom' not in st.session_state:
    st.session_state.item_para_zoom = None
if 'classes_selecionadas_dict' not in st.session_state:
    st.session_state.classes_selecionadas_dict = {}
if 'mrps_selecionados_dict' not in st.session_state:
    st.session_state.mrps_selecionados_dict = {}


# --- Funções Auxiliares ---
@st.cache_data
def carregar_dados():
    caminho_excel = r'C:\Projetos\Controle Materiais Estoque\Controle de Materiais Estoque.xlsx'
    try:
        ...
        st.error(f"ERRO: A planilha de dados não foi encontrada em: {caminho_excel}")
        return None
...
def padronizar_imagem(caminho, tamanho_final=(220, 220)):
    ...
    if not caminho: return placeholder_url
    try:
...
def criar_cartao_material(item):
    ...
    with st.container(border=True, height=420): 
        ...
        with col_zoom:
            # Botão de zoom que usa o session_state
...
if df is not None:
    ...
    # Adiciona as colunas de imagem ao dataframe principal
    ...
    if st.session_state.item_para_zoom:
        # Modo Zoom: Mostra apenas o item selecionado
        ...
        # Modo Galeria: Mostra o título, filtros e os cartões
